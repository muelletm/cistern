...
public class WikiReader {
	...
	WikiReader(String untokenizedFile, String tokenizedFile) {
		this(untokenizedFile, tokenizedFile, Integer.MAX_VALUE);
	}
	
	WikiReader(String untokenizedFile, String tokenizedFile, int limit) {
		untokenized = new LinkedList<String>();
		tokenized = new LinkedList<String>();

		try {
			readFile(untokenizedFile, false, limit);
			readFile(tokenizedFile, true, limit);
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	private void readFile(String filename, Boolean isTokenized, int limit)
			throws IOException {
		File file = new File(filename);
		BZip2CompressorInputStream bzip = new BZip2CompressorInputStream(
		...
		String sentence;
		int count = 0;
		while (scanner.hasNext() && count < limit) { // max number of sentences
														// we consider
			sentence = scanner.next(); // set low here to shorten run time while
										// testing
